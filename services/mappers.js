(function(){
  ...
  function fromWorkOrder(row){ return { id:row.id, workOrderNumber:row.work_order_number, name:row.title, type:row.type, status:row.status, priority:row.priority, date:row.due_date, assignedTo:row.assigned_person || "", projectId:row.project_id, buildingId:row.building_id, spaceId:row.space_id, assetId:row.asset_id, vehicleId:row.vehicle_id, vendorBidId:row.vendor_id, location:row.description || "", notes:row.notes, archivedAt:row.archived_at, updatedAt:row.updated_at }; }
  ...
  function fromTimelineEvent(row){ return { id:row.id, recordType:row.record_type, recordId:row.record_id, relatedRecordType:row.related_record_type, relatedRecordId:row.related_record_id, eventType:row.event_type, actorId:row.actor_id, actorLabel:row.actor_label, note:row.note, metadata:row.metadata || {}, timestamp:row.event_timestamp || row.created_at }; }

  function workOrderPayloadFromForm(form){ return { title:form.name, type:form.frequency && form.frequency !== "One-time" ? "maintenance" : "general", status:dbStatus(form.status, "open"), priority:dbPriority(form.priority), due_date:normalizeDate(form.date), assigned_person:form.assignedTo || null, project_id:form.projectId || null, building_id:form.buildingId || null, space_id:form.spaceId || null, asset_id:form.assetId || null, vehicle_id:form.vehicleId || null, vendor_id:form.vendorId || null, description:form.location || null, notes:form.notes || null }; }
  ...
  function reviewWorkOrderPayloadFromForm(formData){ return { title:formData.get("title") || "Submitted work order", type:formData.get("type") || "general", status:formData.get("status") || "open", priority:formData.get("priority") || "normal", due_date:normalizeDate(formData.get("due_date")), assigned_person:formData.get("assigned_person") || null, project_id:formData.get("project_id") || null, building_id:formData.get("building_id") || null, space_id:formData.get("space_id") || null, asset_id:formData.get("asset_id") || null, vehicle_id:formData.get("vehicle_id") || null, vendor_id:formData.get("vendor_id") || null, description:formData.get("description") || null, notes:formData.get("notes") || null }; }
  ...
  function budgetItemPayloadFromImport(data, vendorId){ return { project_id:data.project_id || null, work_order_id:data.work_order_id || null, vendor_id:vendorId, fuel_receipt_id:data.fuel_receipt_id || null, label:data.label || data.title || data.vendor || data.file_name || "Imported estimate", item_type:data.item_type || "bid", status:"draft", amount:normalizeNumber(data.amount) || 0, date_received:data.date || null, notes:[data.notes, data.review_context ? `Review context: ${data.review_context}` : "", data.extracted_text].filter(Boolean).join("\n\n") || null }; }
  ...
  const Mappers = {
    ...
    fromBuilding, fromSpace, fromAsset, fromProject, fromVendor, fromBudgetItem, fromBudgetBid, fromWorkOrder, fromVehicle, fromFuelReceipt, fromDocument, fromImportReview, fromTimelineEvent,
