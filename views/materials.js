(function(){
  ...
  function materialReviewCard(review){
    ...
    const actions = canManageOperations() && review.status === "Needs Review"
      ? `<div class="actions no-print"><button type="button" onclick="approveMaterialReviewById('${review.id}')">Approve to Budget</button><button class="ghost archive-action" type="button" onclick="archiveSubmissionById('${review.id}')">Reject request</button></div>`
      : "";
